...
public class ItemPickUp : MonoBehaviour
{
    ...
    public void PickUp(GameObject itemObject)
    {
        ...
        int tempStack = holder.currentStack;
        int remainItemStack = inventory.TryAddItem(holder);

        if (remainItemStack <= 0) //음수 또는 0 이면 모두 획득
        {
            Debug.Log($"[성공] {holder.itemData.itemName} 아이템 x{tempStack} 모두 획득함. 남은 개수 없음.");
            animator.SetTrigger("PickUp");
            holder.DestroySelf();
        }
        else if (remainItemStack == holder.currentStack) //남은 개수가 현재 스택과 같으면 하나도 못얻음
        {
            Debug.Log($"[실패] {holder.itemData.itemName} 아이템 획득 불가능. 빈 공간 없음.");
            systemMessageUI.ShowMessage("공간이 부족합니다!");
        }
        else //일부만 획득
        {
            Debug.Log($"{holder.itemData.itemName} 아이템 x{tempStack - remainItemStack} 획득함. 남은 개수: {remainItemStack}.");
            animator.SetTrigger("PickUp");
