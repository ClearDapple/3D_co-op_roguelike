...
public class PlayerLook : MonoBehaviour
{
    ...
    void Update()
    {
        if (Cursor.lockState == CursorLockMode.Locked)
        {
            ...
            cameraTransform.localRotation = Quaternion.Euler(xRotation, 0f, 0f); // 카메라 상하 회전
            playerTransform.Rotate(Vector3.up * mouseX); // 플레이어 좌우 회전
        }
    }

    public void GetReverseMouseLocked()
    {
        if (Cursor.lockState != CursorLockMode.Locked)
        {
            Cursor.lockState = CursorLockMode.Locked;
